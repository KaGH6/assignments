...
-- チャット
WITH GenreRanked AS ( --WITH GenreRanked で名前を付ける。これ全部サブクエリ
    SELECT 
        genres.genre_name,
        programs.program_name,
        AVG(episodes.view_number) AS avg_views,
        RANK() OVER (PARTITION BY genres.genre_name ORDER BY AVG(episodes.view_number) DESC) AS rnk --rnkって名前のランク付け
    FROM genres
INNER JOIN programs ON genres.genre_id = programs.genre_id
INNER JOIN channels ON programs.channel_id = channels.channel_id
INNER JOIN series ON programs.program_id = series.program_id
INNER JOIN seasons ON series.series_id = seasons.series_id
INNER JOIN episodes ON seasons.season_id = episodes.season_id
    GROUP BY genres.genre_name, programs.program_name
)
SELECT genre_name, program_name, avg_views
FROM GenreRanked --サブクエリで作ったところから引っ張ってる
WHERE rnk = 1; --rnkから１番のやつとってる
+------------+--------------+--------------+
| genre_name | program_name | avg_views    |
+------------+--------------+--------------+
| アニメ     | 鬼滅の刃     |  440000.0000 |
| ドラマ     | 半沢直樹     | 3680000.0000 |
| ニュース   | おはよう日本 |  172000.0000 |
